...
function processTemplate() {
  ...
  if (!templateSheet) {
    SpreadsheetApp.getUi().alert('Не найден лист шаблона (не обнаружен лист с заголовками "Название курса / Ссылка на курс / Предлагаемые теги" или с именем, содержащим "' + TEMPLATE_NAME_HINTS.join(', ') + '").');
    return;
  }
  ...
  // keysData:
  // { rows: [{ tag: 'JavaScript', urls: ['a','b'], rowIndex:4, originalTagName: 'JavaScript' }, ...],
  //   tagIndex: { 'javascript': index_in_rows_array, ... } }
  // note: индексируем теги по lowercase для нечувствительности к регистру
